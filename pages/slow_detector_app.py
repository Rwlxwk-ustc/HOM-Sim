...
@st.cache_data
def get_slow_plot_data(tau, N, Omega, Dw1, Dw2, wc1, wc2, I1, I2, dw0, dw1, dw2):
    ...
    for j in range(N):
        E1_spec += n1_j[j] * np.exp(-(omega_axis - w1_j[j])**2 / (2 * dw1**2))
        E2_spec += n2_j[j] * np.exp(-(omega_axis - w2_j[j])**2 / (2 * dw2**2))
    
    tau_range = np.linspace(-5.0, 5.0, 1000)
    overlap_sum = np.sum(n1_j * n2_j)
    
    # 物理推导：当两路光线宽不匹配时，可见度(Visibility)下降因子
    vis_factor = (2 * dw1 * dw2) / (dw1**2 + dw2**2)
    # 频率失配因子
    mismatch_factor = np.exp(-dw0**2 / (dw1**2 + dw2**2))
    # 时域卷积的有效高斯衰减宽度
    eff_dw2 = (dw1**2 * dw2**2) / (dw1**2 + dw2**2)
    
    interference = 0.5 * vis_factor * mismatch_factor * np.exp(-eff_dw2 * tau_range**2 / 2) * overlap_sum
    baseline = 0.25 * (N1_total + N2_total)**2
    
    Pc_final = (baseline - interference) / baseline  
    current_interference = 0.5 * vis_factor * mismatch_factor * np.exp(-eff_dw2 * tau**2 / 2) * overlap_sum
...
with col1:
    ...
    dw1 = st.slider('光路1 线宽 δω1', 0.1, 5.0, 0.8, 0.1)  # 新增控制
...
with col2:
    ...
    dw2 = st.slider('光路2 线宽 δω2', 0.1, 5.0, 0.8, 0.1)  # 新增控制

# 获取数据，并传入 dw1 和 dw2
E1_s, E2_s, t_r, Pc_f, c_Pc = get_slow_plot_data(tau, N, Omega, Dw1, Dw2, 0.0, wc2, 1.0, I2, dw0, dw1, dw2)
...
ax_curve.set_title("Normalized Coincidence Probability $g^{(2)}(\\tau)$")
